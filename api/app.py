...
def get_units():
    ...
    if id: # return one unit
        cur.execute("SELECT * FROM AvailableUnit u JOIN BUILDING b ON u.building_id = b.building_id JOIN UnitListerAccount l ON l.account_id = u.account_id WHERE unit_id = %s;", [id])
        rv = cur.fetchone()
